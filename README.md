...
game:GetService'VirtualUser':Button1Down(Vector3.new(1,1))game:GetService'VirtualUser':Button1Up(Vector3.new(1,1))
...
BlackScreen = true
 ...
 local RenUi = Update:AddWindow("LungPooh Bankkai","10039618734",Enum.KeyCode.RightControl)
...
M:AddSeperatorLeft("Info")
    ...
    M:AddToggleRight("Black Screen [Reduce GPU]",BlackScreen,function(a)
		...
		M:AddSeperatorRight("Banner list")
        local Mysticlist =  M:AddLabelRight("")

function UpdateMysticlist()
    local Mystic = game:GetService("Players").LocalPlayer.PlayerGui.PAGES.SummonPage.Holder.HolderBannerScene.StandardFrame:FindFirstChild("1").Text
    Mysticlist:Set("Mystic : "..Mystic)
end
...
local Legendarylist =  M:AddLabelRight("")

function UpdateLegendarylist()
    local Legendary = game:GetService("Players").LocalPlayer.PlayerGui.PAGES.SummonPage.Holder.HolderBannerScene.StandardFrame:FindFirstChild("2").Text
    Legendarylist:Set("Legendary : "..Legendary)
end
...
local Epiclist =  M:AddLabelRight("")

function UpdateEpiclist()
    local Epic = game:GetService("Players").LocalPlayer.PlayerGui.PAGES.SummonPage.Holder.HolderBannerScene.StandardFrame:FindFirstChild("3").Text
    Epiclist:Set("Epic : "..Epic)
end
...
local checkTimelabel =  M:AddLabelLeft("")

function UpdateCapp()
    local Iqda = game:GetService("Players").localPlayer.PlayerGui.PAGES.SummonPage.Holder.ExpiresFrame.ExpireLabel.Text
    checkTimelabel:Set(Iqda)
end
...
M:AddToggleLeft("Sent WebHook banner",BennerSent,function(a)
    BennerSent = a
end)
...
local NamePlayer = game.Players.LocalPlayer.Name
 local NamePlayer = game.workspace:FindFirstChild(NamePlayer)
    ...
    while wait() do
        pcall(function()
            if TpSummon then
                ...
                local summonCircleCentre = workspace.Lobby.SummonParticles.SummonCircle.Centre.CFrame
                if summonBannerSelect.Visible == false and game:GetService("Players").LocalPlayer.PlayerGui.PAGES.SummonPage.Visible == false then
                    Tp(summonCircleCentre)
                    game:GetService("VirtualInputManager"):SendKeyEvent(true, "E", false, game)
					game:GetService("VirtualInputManager"):SendKeyEvent(false, "E", false, game)
                elseif summonBannerSelect.Visible == true then
					local virtualUser = game:GetService("VirtualUser")
                    virtualUser:Button1Down(Vector2.new(1, 1))
                    virtualUser:Button1Up(Vector2.new(1, 1))
                    local bannerButton = summonBannerSelect.Options.BannerButton
                    bannerButton.Size = UDim2.new(0, 1000, 0, 1000)
                    bannerButton.Position = UDim2.new(-2, 0, -5, 0)
...
spawn(function()
    while wait() do
        pcall(function()
            if BennerSent then
                if game:GetService("Players").localPlayer.PlayerGui.PAGES.SummonPage.Holder.ExpiresFrame.ExpireLabel.Text == "Expires In: 59:54" and ChackTime then
...
for i, v in pairs(game.Players.LocalPlayer.PlayerGui.PAGES.SummonPage.ChancesFrame.UnitHolder:GetChildren()) do
    if v.Name == "UnitGridPrefab" then
        print("Found UnitGridPrefab with LayoutOrder: " .. v.LayoutOrder)
        if v.LayoutOrder == -7 then
            if v:FindFirstChild("Button") and v.Button:FindFirstChild("UnitNameLabel") then
                Rare1 = v.Button.UnitNameLabel.Text
			end
        elseif v.LayoutOrder == -6 then
            if v:FindFirstChild("Button") and v.Button:FindFirstChild("UnitNameLabel") then
                Rare2 = v.Button.UnitNameLabel.Text
                      
            end
        elseif v.LayoutOrder == -5 then
            if v:FindFirstChild("Button") and v.Button:FindFirstChild("UnitNameLabel") then
                Rare3 = v.Button.UnitNameLabel.Text
    ...
    pcall(function()
        ...
        local data = {
            ...
            ["embeds"] = {
                {
                    ["author"] = {
                        ["name"] = "by ballnaja#3764"
                    },
                    ["type"] = "rich",
                    ["title"] = "Anime Defenders [SUMMER]",
        ...
        local request = http_request or request or syn.request or function(params)
            return game:GetService("HttpService"):PostAsync(params.Url, params.Body, Enum.HttpContentType.ApplicationJson, false)
        end
...
ChackTime = false
elseif game:GetService("Players").localPlayer.PlayerGui.PAGES.SummonPage.Holder.ExpiresFrame.ExpireLabel.Text == "Expires In: 59:54" and ChackTime == false then
    ChackTime = true
        end
end
end)
